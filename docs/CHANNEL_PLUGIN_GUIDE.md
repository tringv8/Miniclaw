...
Build a custom miniclaw channel in three steps: subclass, package, install.

> **Note:** We recommend developing channel plugins against a source checkout of miniclaw (`pip install -e .`) rather than a PyPI release, so you always have access to the latest base-channel features and APIs.

## How It Works

miniclaw discovers channel plugins via Python [entry points](https://packaging.python.org/en/latest/specifications/entry-points/). When `miniclaw gateway` starts, it scans:

1. Built-in channels in `miniclaw/channels/`
2. External packages registered under the `miniclaw.channels` entry point group
...
```
miniclaw-channel-webhook/
├── miniclaw_channel_webhook/
...
```python
# miniclaw_channel_webhook/__init__.py
from miniclaw_channel_webhook.channel import WebhookChannel

__all__ = ["WebhookChannel"]
```

```python
# miniclaw_channel_webhook/channel.py
import asyncio
...
# pyproject.toml
[project]
name = "miniclaw-channel-webhook"
version = "0.1.0"
dependencies = ["miniclaw", "aiohttp"]

[project.entry-points."miniclaw.channels"]
webhook = "miniclaw_channel_webhook:WebhookChannel"
...
```bash
pip install -e .
miniclaw plugins list      # verify "Webhook" shows as "plugin"
miniclaw onboard           # auto-adds default config for detected plugins
```
...
```bash
miniclaw channels login <channel_name>
miniclaw channels login <channel_name> --force  # re-authenticate
```
...
| `default_config()` (classmethod) | Returns default config dict for `miniclaw onboard`. Override to declare your fields. |
...
Override `default_config()` so `miniclaw onboard` auto-populates `config.json`:
...
| What | Format | Example |
|------|--------|---------|
| PyPI package | `miniclaw-channel-{name}` | `miniclaw-channel-webhook` |
| Entry point key | `{name}` | `webhook` |
| Config section | `channels.{name}` | `channels.webhook` |
| Python package | `miniclaw_channel_{name}` | `miniclaw_channel_webhook` |

## Local Development

```bash
git clone https://github.com/you/miniclaw-channel-webhook
cd miniclaw-channel-webhook
pip install -e .
miniclaw plugins list    # should show "Webhook" as "plugin"
miniclaw gateway         # test end-to-end
```

## Verify

```bash
$ miniclaw plugins list
