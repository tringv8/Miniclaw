...
def split_message(content: str, max_len: int = 2000) -> list[str]:
    ...
    Args:
        ...
        max_len: Maximum length per chunk for chat platform compatibility.
...
def build_status_content(
    *,
    ...
    return "\n".join([
        f"\U0001f408 miniclaw v{version}",
...
def sync_workspace_templates(workspace: Path, silent: bool = False) -> list[str]:
    ...
    tpl = Path(__file__).resolve().parent.parent.parent / "workspace" / "templates"
    if not tpl.is_dir():
        return []
