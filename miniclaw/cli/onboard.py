"""Interactive onboarding questionnaire for miniclaw."""
...
from miniclaw.cli.models import (
    format_token_count,
...
def _show_main_menu_header() -> None:
    ...
    console.print(
        Align.center(f"{__logo__} [bold cyan]miniclaw[{__version__}][/bold cyan]")
    )
    console.print()
...
def _try_auto_fill_context_window(model: BaseModel, new_model_name: str) -> None:
    ...
    Note:
        This function imports AgentDefaults from miniclaw.config.schema to get
    ...
    from miniclaw.config.schema import AgentDefaults
    ...
    if context_limit:
        ...
        console.print(f"[green]+ Tự động điền đầy cửa sổ ngữ cảnh (Context Window): {format_token_count(context_limit)} tokens[/green]")
    else:
        console.print("[dim](i) Không thể tự động điền kích thước cửa sổ ngữ cảnh (mô hình không có trong cơ sở dữ liệu)[/dim]")
...
def _configure_providers(config: Config) -> None:
    ...
    while True:
        try:
            ...
            console.print("\n[dim]Đang quay lại menu chính...[/dim]")
            break
...
def _get_channel_info() -> dict[str, tuple[str, type[BaseModel]]]:
    ...
    for name, channel_cls in discover_all().items():
        try:
            mod = importlib.import_module(f"miniclaw.channels.{name}")
...
def _configure_channel(config: Config, channel_name: str) -> None:
    ...
    if config_cls is None:
        console.print(f"[red]Không tìm thấy lớp cấu hình cho {display_name}[/red]")
        return
...
def run_onboard(initial_config: Config | None = None) -> OnboardResult:
    ...
    while True:
        ...
        try:
            answer = _get_questionary().select(
                "Bạn muốn cấu hình (thiết lập) nội dung gì?",
                choices=[
                    "[P] LLM Provider",
                    "[C] Chat Channel",
                    "[A] Agent Settings",
