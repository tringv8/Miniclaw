...
from miniclaw.cli.commands import _make_provider, app
from miniclaw.config.schema import Config
from miniclaw.providers.openai_codex_provider import _strip_model_prefix
...
def mock_paths():
    ...
    with patch("miniclaw.config.loader.get_config_path") as mock_cp, \
         patch("miniclaw.config.loader.save_config") as mock_sc, \
         patch("miniclaw.config.loader.load_config") as mock_lc, \
         patch("miniclaw.cli.commands.get_workspace_path") as mock_ws:
...
def test_onboard_fresh_install(mock_paths):
    ...
    assert "miniclaw is ready" in result.stdout
...
def test_onboard_interactive_discard_does_not_save_or_create_workspace(mock_paths, monkeypatch):
    ...
    from miniclaw.cli.onboard import OnboardResult

    monkeypatch.setattr(
        "miniclaw.cli.onboard.run_onboard",
...
def test_onboard_uses_explicit_config_and_workspace_paths(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("miniclaw.channels.registry.discover_all", lambda: {})
...
def test_onboard_wizard_preserves_explicit_config_in_next_steps(tmp_path, monkeypatch):
    ...
    compact_output = stripped_output.replace("\n", "")
    resolved_config = str(config_path.resolve())
    assert f'miniclaw agent -m "Hello!" --config {resolved_config}' in compact_output
    assert f"miniclaw gateway --config {resolved_config}" in compact_output
...
def test_openai_compat_provider_passes_model_through():
    ...
    with patch("miniclaw.providers.openai_compat_provider.AsyncOpenAI"):
...
def test_make_provider_passes_extra_headers_to_custom_provider():
    ...
    with patch("miniclaw.providers.openai_compat_provider.AsyncOpenAI") as mock_async_openai:
...
def mock_agent_runtime(tmp_path):
    ...
    with patch("miniclaw.config.loader.load_config", return_value=config) as mock_load_config, \
         patch("miniclaw.cli.commands.sync_workspace_templates") as mock_sync_templates, \
         patch("miniclaw.cli.commands._make_provider", return_value=object()), \
         patch("miniclaw.cli.commands._print_agent_response") as mock_print_response, \
         patch("miniclaw.bus.queue.MessageBus"), \
         patch("miniclaw.cron.service.CronService"), \
         patch("miniclaw.agent.loop.AgentLoop") as mock_agent_loop_cls:
...
def test_agent_config_sets_active_path(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "miniclaw.config.loader.set_config_path",
        lambda path: seen.__setitem__("config_path", path),
    )
    monkeypatch.setattr("miniclaw.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("miniclaw.cli.commands.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("miniclaw.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("miniclaw.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("miniclaw.cron.service.CronService", lambda _store: object())
    ...
    monkeypatch.setattr("miniclaw.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("miniclaw.cli.commands._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_agent_uses_workspace_directory_for_cron_store(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("miniclaw.config.loader.set_config_path", lambda _path: None)
    monkeypatch.setattr("miniclaw.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("miniclaw.cli.commands.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("miniclaw.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("miniclaw.bus.queue.MessageBus", lambda: object())
    ...
    monkeypatch.setattr("miniclaw.cron.service.CronService", _FakeCron)
    monkeypatch.setattr("miniclaw.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("miniclaw.cli.commands._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_agent_workspace_override_does_not_migrate_legacy_cron(
    ...
    monkeypatch.setattr("miniclaw.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("miniclaw.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("miniclaw.config.paths.get_cron_dir", lambda: legacy_dir)
...
def test_gateway_uses_workspace_from_config_by_default(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("miniclaw.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr(
        "miniclaw.cli.commands.sync_workspace_templates",
        lambda path: seen.__setitem__("workspace", path),
    )
    monkeypatch.setattr(
        "miniclaw.cli.commands._make_provider",
...
def test_gateway_uses_workspace_directory_for_cron_store(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("miniclaw.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("miniclaw.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("miniclaw.session.manager.SessionManager", lambda _workspace: object())
    ...
    monkeypatch.setattr("miniclaw.cron.service.CronService", _StopCron)
...
def test_migrate_cron_store_moves_legacy_file(tmp_path: Path) -> None:
    ...
    from miniclaw.cli.commands import _migrate_cron_store
    ...
    with patch("miniclaw.config.paths.get_cron_dir", return_value=legacy_dir):
