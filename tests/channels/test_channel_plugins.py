...
from miniclaw.config.schema import ChannelsConfig
...
def test_discover_plugins_loads_entry_points():
    from miniclaw.channels.registry import discover_plugins
...
async def test_manager_loads_plugin_from_dict_config():
    ...
    with patch(
        "miniclaw.channels.registry.discover_all",
...
def test_channels_login_uses_discovered_plugin_class(monkeypatch):
    ...
    monkeypatch.setattr("miniclaw.config.loader.load_config", lambda: Config())
    monkeypatch.setattr(
        "miniclaw.channels.registry.discover_all",
...
async def test_send_with_retry_retries_on_failure():
    ...
    with patch("miniclaw.channels.manager.asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
        await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_send_with_retry_no_retry_when_max_is_zero():
    ...
    with patch("miniclaw.channels.manager.asyncio.sleep", new_callable=AsyncMock):
        await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_send_with_retry_propagates_cancelled_error_during_sleep():
    ...
    with patch("miniclaw.channels.manager.asyncio.sleep", side_effect=cancel_during_sleep):
        with pytest.raises(asyncio.CancelledError):
            await mgr._send_with_retry(mgr.channels["failing"], msg)
