...
interface KimiCredentialCardProps {
...
export function KimiCredentialCard({
  status,
  activeAction,
  token,
  onTokenChange,
  ...
  onSaveToken,
  onAskLogout,
}: KimiCredentialCardProps) {
  const { t } = useTranslation()
  const actionBusy = activeAction !== ""
  const tokenLoading = activeAction === "moonshot:token"
  ...
  return (
    <CredentialCard
      ...
      description={status?.help_text || t("credentials.providers.moonshot.description")}
          ...
          <div className="flex h-full flex-col gap-3">
            <div className="flex h-full items-center gap-2">
              <Input
                ...
                type="password"
                placeholder={t("credentials.fields.kimiToken")}
              />
              <Button
                size="sm"
                className="w-fit"
