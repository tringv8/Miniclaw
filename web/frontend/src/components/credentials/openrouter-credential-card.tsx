    ...
    <CredentialCard
      title={
        <span className="inline-flex items-center gap-2">
          <span className="border-muted bg-background inline-flex size-6 items-center justify-center rounded-full border p-1">
            <ProviderIcon providerKey="openrouter" providerLabel="OpenRouter" />
          </span>
          <span>OpenRouter</span>
        </span>
      }
      description={status?.help_text || t("credentials.providers.openrouter.description")}
      ...
      footer={
        status?.logged_in && status?.supports_logout ? (
          <Button
            ...
            className="text-destructive hover:bg-destructive/10 hover:text-destructive"
          >
            {activeAction === "openrouter:logout" && (
              <IconLoader2 className="size-4 animate-spin" />
            )}
